...
#
#   dist/esp32-nut-ecoflow-<version>-factory.bin   flash at 0x0 (USB, first time)
#   dist/esp32-nut-ecoflow-<version>.bin           app only (web OTA update)
#
...
idf.py build
idf.py merge-bin -o "esp32-nut-ecoflow-factory.bin" >/dev/null
...
EMBEDDED=$(python3 -c "
import sys
d = open('build/esp32-nut-ecoflow.bin','rb').read()
...
mkdir -p dist
cp build/esp32-nut-ecoflow-factory.bin "dist/esp32-nut-ecoflow-$V-factory.bin"
cp build/esp32-nut-ecoflow.bin         "dist/esp32-nut-ecoflow-$V.bin"
...
for f in "dist/esp32-nut-ecoflow-$V-factory.bin" "dist/esp32-nut-ecoflow-$V.bin"; do
